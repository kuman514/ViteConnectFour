...
- 추가 기능
  ...
  - 음향 추가
    - 게임이나 리플레이에서 소리가 발생하는 조건
      ...
      - 초기화 또는 리플레이 파일 로딩
  - ~~리플레이 기능 구현~~ (완료)
    - ~~버튼형 그리드 타일과 리플레이 상태 관리에 쓰일 스토어는 게임 기능 구현에 썼던 타일을 재활용~~ (완료)
      - ~~단 리플레이에서는 모든 그리드 타일의 클릭을 비활성화 해야 함~~ (완료)
      - ~~이전 페이지, 다음 페이지로 넘어간다~~ (완료)
    - ~~게임 기능에 리플레이 파일 생성 버튼 추가~~ (완료)
      - ~~승자 결정 후 사용 가능하도록 만든다~~ (완료)
    - ~~조합한 내용을 리플레이 페이지에 반영~~ (완료)
